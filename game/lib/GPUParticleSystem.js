...
THREE.GPUParticleSystem = function ( options, blending, fade ) {
	...
	var GPUParticleShader = {
		...
		vertexShader: [
			...
			'	lifeLeft = timeElapsed / lifeTime ;',

			'	gl_PointSize = ( uScale * size );',
			'   if (!fade) ',
			'		gl_PointSize = ( uScale * size ) * (1. - lifeLeft);',
			'',
			...
			'	vec3 noise = texture2D( tNoise, vec2( ( cos(timeElapsed * 0.0005 + positionStart.x) ), ( sin(timeElapsed * 0.00015 + positionStart.y) ) ) ).rgb;',
			'	vec3 noiseVel = ( noise.rgb - 0.5 ) * 0.01 * timeElapsed;',
			'   noiseVel.z *= 0.1;',

			'	newPosition = newPosition + vec3( noiseVel * ( turbulence * 5.0 ));',
		...
		fragmentShader: [
			...
			'	gl_FragColor = vec4( vColor.rgb * tex.rgb * 2.0, tex.a * alpha );',
	...
	this.particleNoiseTex = this.PARTICLE_NOISE_TEXTURE || textureLoader.load( 'tex/perlin-512.png' );
