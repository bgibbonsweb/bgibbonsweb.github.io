...
const MathUtils = {};
MathUtils.isPowerOfTwo = function(n) {
	return n % 2 == 0;
}
...
var Water = function ( geometry, options ) {
	...
	var mirrorShader = {
		...
		fragmentShader: [
			...
			'   reflectionSample *= vec3(0.12, 0.1, 0.08) * 5.0; ',
			'	float theta = max( dot( eyeDirection, surfaceNormal ), 0.0 );',
			'	float rf0 = 0.3;',
			'	float reflectance = rf0 + ( 1.0 - rf0 ) * pow( ( 1.0 - theta ), 5.0 );',
			'	vec3 scatter = max( 0.0, dot( surfaceNormal, eyeDirection ) ) * waterColor;',
			'	vec3 albedo = mix( ( sunColor * diffuseLight * 0.3 + scatter ), ( reflectionSample * 0.9 + reflectionSample ), reflectance);',
